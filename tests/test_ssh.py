...
def test_build_ssh_command_uses_strict_host_verification() -> None:
    settings = load_settings(
        {
            ...
            "RPI_MCP_SSH_IDENTITY_FILE": "test-fixtures/rpi",
            "RPI_MCP_KNOWN_HOSTS_FILE": "test-fixtures/known_hosts",
        }
    )
    ...
    assert command == [
        ...
        "2222",
        "-i",
        str(Path("test-fixtures/rpi")),
        "-o",
        f"UserKnownHostsFile={Path('test-fixtures/known_hosts')}",
        "operator@192.0.2.5",
        "uname -a",
    ]
    ...
    assert rpi3_command[-2:] == ["operator3@192.0.2.3", "uname -m"]
